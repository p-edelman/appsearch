...
public class SmartIcon
    ...
    private void updateWidgets(Context context) {
        ...
        for (int widget_num = 0; widget_num < widget_ids.length; widget_num++) {
            ...
            if (app_resources != null) {
                // For responding to touch, we first need to create an internal intent (that can be
                // caught by onReceive()). Then we wrap this intent in a PendingIntent, that we can
                // bind to an icon.
                // Because the intents for all icons look the same (they only differ in the extra
                // data), Android will not create a new PendingIntent object for each icon.
                // Therefore, we need to set a different request code for all of them AND set the
                // FLAG_UPDATE_CURRENT, which will update the current PendingIntent with the new
                // intent.
